...
class PlotGui():
    ...
    def __init__(self,root,fig,yrange=np.array([0,0.5]),verbose=0):
        ...
        # ylim scales    
        label = Label(side_frame,text='Y limits',font=self.smallfont)
        label.pack(fill=BOTH)
        
        # one y limit scale that effects all subplots
        self.plotlist = self.fig.axes[:]
        
        ylim_scale = [self.setup_lim_scales(side_frame,
                                            subplots=self.plotlist,
                                            scale_range=self.yrange)]

        for myscale in ylim_scale:
            low, high = self.plotlist[0].get_ylim()
            myscale.set(high) # set to current upper limit
            myscale.pack()
            #myscale.place(in_=side_frame, anchor=CENTER, relx=.5, rely=0.3)
